...
public class trans_seed_manager {

	// DEFAULT : JAVA = BIG_ENDIAN
	private static int ENDIAN = Common.BIG_ENDIAN;
	private static trans_util_manager utilm = new trans_util_manager();
	...
	public static byte[] seed_enc_card(byte[] pbData){
		
		byte pbUserKey[]  = {(byte)0xD1, (byte)0xE3, (byte)0x33, (byte)0x8F, (byte)0x08, (byte)0x17, (byte)0x79, 
				(byte)0xF1, (byte)0xE9, (byte)0xF3, (byte)0x94, (byte)0x37, (byte)0x0A, (byte)0xD4, (byte)0x05, (byte)0x89};
		...
		return defaultCipherText;
	}
	
	public static String seed_enc_str(String cardno){
		String rtn = null;
		StringBuffer stbcardenc = new StringBuffer();
		
		byte pbUserKey[]  = {(byte)0xD1, (byte)0xE3, (byte)0x33, (byte)0x8F, (byte)0x08, (byte)0x17, (byte)0x79, 
				(byte)0xF1, (byte)0xE9, (byte)0xF3, (byte)0x94, (byte)0x37, (byte)0x0A, (byte)0xD4, (byte)0x05, (byte)0x89};

		byte pbData[]     = cardno.getBytes();
		byte bszIV[] = {
				(byte)0x026, (byte)0x08d, (byte)0x066, (byte)0x0a7,
				(byte)0x035, (byte)0x0a8, (byte)0x01a, (byte)0x081,
				(byte)0x06f, (byte)0x0ba, (byte)0x0d9, (byte)0x0fa,
	    ...
	    int PLAINTEXT_LENGTH = pbData.length;
		int CIPHERTEXT_LENGTH = ((PLAINTEXT_LENGTH / 16) * 16) + 16;	
	    
	    byte[] defaultCipherText = SEED_CBC_Encrypt(pbUserKey, bszIV, pbData, 0, PLAINTEXT_LENGTH);
		
	    for (int i=0; i<CIPHERTEXT_LENGTH; i++){
	    	stbcardenc.append(utilm.AddZero(Integer.toHexString(0xff&defaultCipherText[i]), 2));
	    }
	    
	    rtn = stbcardenc.toString();
	    
		return rtn;
	}
	
	public static String seed_dec_card(String tmpByte){
		String rtn = null;
		try {
			...
			byte[] planByte	= utilm.hexStringToByteArray(tmpByte);
	
		    int PLAINTEXT_LENGTH = planByte.length;
			
		    byte[] defaultCipherText = SEED_CBC_Decrypt(pbUserKey, bszIV, planByte, 0, PLAINTEXT_LENGTH);
			
		    
		    for (int i=0; i<defaultCipherText.length; i++){
		    	stbcardenc.append((char) defaultCipherText[i]);
		    }
